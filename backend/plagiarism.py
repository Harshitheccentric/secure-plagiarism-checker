"""
Plagiarism detection using KMP algorithm
Adapted from the original kmp.py for web application use
"""
...
def run_plagiarism_check(decrypted_dir, method='word_based'):
    """
    Run plagiarism detection on all files in decrypted directory
    
    Args:
        decrypted_dir (str): Path to directory containing decrypted files
        method (str): Detection method ('word_based', 'char_based', 'line_based')
    
    Returns:
        list: List of comparison results
    """
    # Get all decrypted files
    decrypted_files = glob.glob(os.path.join(decrypted_dir, "*.txt"))
    ...
    for file1, file2 in combinations(file_names, 2):
        ...
        similarity_data = plagiarism_score(content1, content2, method)
